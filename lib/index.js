...
/**
 * Returns the right value from sorting two strings.
 *
 * @param {string} a
 * @param {string} b
 * @return {number}
...
module.exports = function (options) {
    ...
    return function (files, metalsmith, done) {
        ...
        function assignRelationships(ancestry) {
            ...
            // Find the parent
            folder = path.resolve("/", ancestry.path, "..").slice(1);

            if (folder !== "") {
                parentFolder = path.resolve("/", folder, "..").slice(1);
            ...
            Object.keys(byFolder).forEach(function (folderPath) {
                if (path.resolve("/", folderPath, "..").slice(1) === folder) {
        ...
        function findSiblings(filename, file) {
            ...
            folder = path.resolve("/", filename, "..").slice(1);
