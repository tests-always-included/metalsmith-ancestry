 ...
 *
 * @typedef {Object.<string,metalsmithFile>} metalsmithFileMap
 */
 ...
 * @property {module:metalsmith-ancestry~metalsmithFileMap} [childrenByName] Find specific child folder.
...
function assignChildren(filesByFolder, sortFn, options, ancestry) {
    var children, childrenByName, folder, i, keys;
    ...
    // of each lineage. Same with the object.
    children = [];
    childrenByName = {};
    folder = path.posix.resolve("/", ancestry.path, "..").slice(1);
    keys = Object.keys(filesByFolder);
    ...
    for (i = 0; i < keys.length; i += 1) {
        ...
        if (folderPath !== folder) {
            ...
            if (theirParent === folder) {
                const file = filesByFolder[folderPath][0];

                children.push(file);
                childrenByName[folderPath.split("/").pop()] = file;
            }
        }
    }
    ...
    for (i = 0; i < ancestry.members.length; i += 1) {
        ...
        member[options.ancestryProperty].childrenByName = childrenByName;
